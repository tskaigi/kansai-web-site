...
import { type SponsorList, sponsorList } from "@/lib/data/sponsorList";
  ...
  return (
    <div className="">
      ...
      {plans
        ...
        .map((plan) => {
          const list = sponsorList[plan].filter((sponsor) => !sponsor.draft);
          if (list.length === 0) {
            return null;
          }
          return (
            <div key={plan}>
              <h3
                className={`divider mb-8 text-2xl font-bold ${sponsorVariants[plan].divider} `}
              >
                {plan} Sponsors
              </h3>
        ...
        })}
      {
        <div>
          <h3
            className={`divider mb-8 text-2xl font-bold ${sponsorVariants.Bronze.divider} `}
          >
            Bronze Sponsors
          </h3>
          <div
          ...
          >
            {sponsorList.Bronze.filter((sponsor) => !sponsor.draft).map(
              (sponsor) => (
                <Link
                  key={sponsor.name}
                  href={sponsor.logoLink}
